  ...
  var ROUTER = createRouter({
    ...
    onActivate: function (name, previous, opts) {
      /* A nested route does not survive navigating away from the screen
         that hosts it. The tool host releases its own stack in onLeave;
         these two are still held here because the account and
         authentication screens have not been extracted yet.

         Re-activating the screen you are already on is a refresh, not a
         departure, so neither is cleared in that case. */
      if (name !== 'account' && previous === 'account') { accountRoute = null; accountStack.length = 0; }
      if (name !== 'auth' && previous === 'auth') { authRoute = null; authStack.length = 0; }
