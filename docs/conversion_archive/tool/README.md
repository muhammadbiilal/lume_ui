...
| `count_web_assertions.mjs` | The canonical count of the web oracle's assertions, per suite and in total. It exists because the ten suites print two different `ok` shapes — nine with whitespace, `verify.js` with a colon — so a grep written for one silently loses the other, which is how a report once said 703 against 715 |
| `probe_notifications.mjs` | Drives the shell to the notification centre on a pinned clock and reports the badge *and* the unread rows. The badge counts the notification sources that survive a profile, so it is 13 in Pakistan, 12 with the content switches off and 10 abroad; this is what the Flutter fixture's per-state values were read from |
