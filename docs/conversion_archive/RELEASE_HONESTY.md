...
| Reader's own records, nothing seeded | `readerRecords` | 4 — ledger, installments, committee, babybudget | all four are converted (C90, C94, C96, C97): each shows only what the reader wrote, so none is sample; the storage claim is still the store's — "Kept until you close Lume" in development and release builds, because the in-memory store is not durable (C74) | `isSample` false |
| Every figure is what the reader pressed | `inputOnly` | 8 — age, tipsplit, loan, compound, stopwatch, **calculator**, **tasbih**, **focus** | wave 3 adds two: Calculator's readout and history are the keys the reader pressed, worked out exactly; Tasbih's count, round and rounds are their own taps once the fixture history is dropped (C98). C99 adds Focus Timer, whose lengths and finished stretches are equally the reader's — see the row below for the build it is *not* true of | `isSample` false |
| Sample data only where the build reproduces it | `sampleInParityOnly` | 1 — **focus** | Focus Timer's invented history — 75 minutes, a five-day streak, three sessions, a seven-bar week — is drawn only under `reproducesReference`. The parity capture discloses it; development and release have no sample data on that screen to disclose, and said so anyway until C99 | `isSample` = `reproducesReference` |
