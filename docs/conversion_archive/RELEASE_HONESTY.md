...
| Every figure is what the reader pressed | `inputOnly` | 7 — age, tipsplit, loan, compound, stopwatch, **calculator**, **tasbih** | wave 3 adds two: Calculator's readout and history are the keys the reader pressed, worked out exactly; Tasbih's count, round and rounds are their own taps once the fixture history is dropped (C98) | `isSample` false |
| Worked out here from a compiled-in table | `computed` | 2 — sunmoon, **worldclock** | World Clock's times come from the IANA database in the binary and the device's clock. `'IANA time zones'` had to join `staticSources` for the source bar to say so instead of "Sample data" (C98) | `isSample` false |
