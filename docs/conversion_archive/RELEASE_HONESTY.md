...
| Reader's own records, nothing seeded | `readerRecords` | 1 — ledger | **ledger** is converted (C90): it shows only what the reader wrote, so it is not sample; its storage claim is still the store's — "Kept until you close Lume" in a release build, because the in-memory store is not durable (C74) | `isSample` false |
