...
- **Notifications** — a row opens its tool on the item it is about (Parcel's consignment, the flight, the train — the web's `toolstate` deep links, written into the tool's session), a grouped row only marks itself read, and the action button marks the item actioned (`markActioned`). The centre and the banner open through one function. Markets' "detail" link waits on Markets itself.
