...
void main() {
  group('model and codec', () {
    test('a fast round-trips through its fields', () {
      ...
      final FastEntry e = h.log(kToday, kind: FastingKind.voluntary, kept: true);
    ...
    test('a record of the wrong schema is a defect, not a crash', () {
      final FastingHarness h = FastingHarness();
      addTearDown(h.dispose);
      h.raw('fasting.entry', 'not-a-uuid', <String, Object?>{'schema': 'lume.other/1'});
  ...
  group('update', () {
    test('correcting kept from true to false moves it out of the kept count', () {
      final FastingHarness h = FastingHarness();
      addTearDown(h.dispose);
    ...
    test('a stale version is a conflict, not a silent overwrite', () {
      ...
      h.repo.updateFast(e.id, date: e.date, kind: e.kind, kept: false, expectVersion: e.version);
      final FastingResult<FastingWrite> r = h.repo.updateFast(
        e.id,
        date: e.date,
  ...
  group('delete and undo', () {
    test('deleting removes the fast', () {
      ...
      addTearDown(h.dispose);
      final FastEntry e = h.log(kToday);
      final FastingResult<FastingWrite> r = h.repo.deleteFast(e.id, expectVersion: e.version);
      expect(r.ok, isTrue);
      expect(h.repo.view().entries, isEmpty);
    });
    ...
    test('undo restores a deleted fast', () {
      ...
      addTearDown(h.dispose);
      final FastEntry e = h.log(kToday);
      final FastingResult<FastingWrite> deleted = h.repo.deleteFast(e.id, expectVersion: e.version);
  ...
  group('FastingInsights.compute — the reference has no equivalent to match', () {
    test('no fasts logged: nothing to show, nothing invented', () {
      final FastingInsights i = FastingInsights.compute(entries: const <FastEntry>[], today: kToday);
      ...
      expect(i.heat.every((FastingHeatDay d) => d.state == FastingDayState.none), isTrue);
    });
    ...
    test('a fast logged in a different month does not count toward this month', () {
      final FastingHarness h = FastingHarness();
      addTearDown(h.dispose);
    ...
    test('a missed fast counts toward missed, never invented as the complement of kept', () {
      final FastingHarness h = FastingHarness();
      addTearDown(h.dispose);
    ...
    test('the streak is the run of consecutive kept entries ending at the most recent log', () {
      final FastingHarness h = FastingHarness();
      addTearDown(h.dispose);
      h.log(d(9, 1), kept: true);
      h.log(d(9, 4), kept: true); // not calendar-adjacent — a voluntary fast is not daily
      h.log(d(9, 6), kept: true);
      final FastingInsights i = h.insights(kToday);
      expect(i.currentStreak, 3);
    });

    test('a missed entry breaks the streak even when kept entries follow it earlier', () {
      final FastingHarness h = FastingHarness();
      addTearDown(h.dispose);
      h.log(d(9, 1), kept: true);
    ...
    test('the streak is zero when the most recently logged fast was missed', () {
      final FastingHarness h = FastingHarness();
      addTearDown(h.dispose);
      h.log(d(9, 1), kept: true);
      h.log(d(9, 2), kept: false);
    ...
    test('recent lists every logged entry, most recent date first', () {
      ...
      final FastingInsights i = h.insights(kToday);
      expect(i.recent.map((FastEntry e) => e.date), <LumeDate>[d(9, 5), d(9, 3), d(9, 1)]);
    });

    test('the heat window covers the trailing 30 days ending today, over real entries only', () {
      final FastingHarness h = FastingHarness();
      addTearDown(h.dispose);
      ...
      final FastingHeatDay missedDay = i.heat.firstWhere((FastingHeatDay d) => d.date == kToday.addDays(-2));
      expect(missedDay.state, FastingDayState.missed);
      final FastingHeatDay untouched = i.heat.firstWhere((FastingHeatDay d) => d.date == kToday.addDays(-1));
