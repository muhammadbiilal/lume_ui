...
void main() {
  ...
  group('logging a fast', () {
    testWidgets(
      "the empty state's call to action logs today as a kept voluntary fast",
    ...
    testWidgets('the primary action logs a second fast the same way', (
      WidgetTester t,
    ) async {
      final FastingWorld w = FastingWorld();
  ...
  group('editing and deleting', () {
    testWidgets('tapping a recent row opens it for editing', (
      WidgetTester t,
    ) async {
      final FastingWorld w = FastingWorld();
      final FastEntry e = w.log(
        kToday,
    ...
    testWidgets('deleting a fast removes it and Undo brings it back', (
      WidgetTester t,
    ) async {
      final FastingWorld w = FastingWorld();
  ...
  group('the reference\'s own figures, computed for real', () {
    testWidgets(
      'the summary shows a real month count and a real streak, never the reference\'s literals',
      (WidgetTester t) async {
        final FastingWorld w = FastingWorld();
        ...
        expect(
          s.stats.firstWhere((LumeStat st) => st.label.isNotEmpty).value,
          isNotEmpty,
        );
        w.dispose();
      },
    ...
    testWidgets('the kind filter narrows the recent list to one kind', (
      WidgetTester t,
    ) async {
      final FastingWorld w = FastingWorld();
      final FastEntry voluntary = w.log(
        kToday.addDays(-1),
        kind: FastingKind.voluntary,
      );
      ...
      expect(
        find.byKey(LumeFastingTool.recentRowKey(voluntary.id)),
        findsOneWidget,
      );
      expect(
        find.byKey(LumeFastingTool.recentRowKey(makeup.id)),
        findsOneWidget,
      );
  ...
  group('locale', () {
    testWidgets('right to left, a recent row runs from the right', (
      WidgetTester t,
    ) async {
      final FastingWorld w = FastingWorld();
      ...
      expect(
        Directionality.of(
          t.element(
            inKey(LumeFastingTool.recentKey, find.byType(LumeRichRow)).first,
          ),
        ),
        TextDirection.rtl,
      );
  ...
  group('200% text scale', () {
    testWidgets("this tool's own cards do not overflow", (
      WidgetTester t,
    ) async {
      final FastingWorld w = FastingWorld();
    ...
    testWidgets('the empty state does not overflow either', (
      WidgetTester t,
    ) async {
      final FastingWorld w = FastingWorld();
      await pumpFasting(t, w, textScale: 2);
