...
void main() {
  group('model and codec', () {
    ...
    test('a record of the wrong schema is a defect, not a crash', () {
      final PrayTrackHarness h = PrayTrackHarness();
      addTearDown(h.dispose);
      h.raw(PrayTrackCollections.checkins, 'aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa', <String, Object?>{
        'schema': 'lume.other/1',
      });
    ...
    test('an unrecognised prayer name is a defect, not a crash', () {
      final PrayTrackHarness h = PrayTrackHarness();
      addTearDown(h.dispose);
      h.raw(PrayTrackCollections.checkins, 'bbbbbbbb-bbbb-4bbb-8bbb-bbbbbbbbbbbb', <String, Object?>{
    ...
    test('a malformed date is a defect, not a crash', () {
      final PrayTrackHarness h = PrayTrackHarness();
      addTearDown(h.dispose);
      h.raw(PrayTrackCollections.checkins, 'cccccccc-cccc-4ccc-8ccc-cccccccccccc', <String, Object?>{
        'schema': kPrayTrackSchema,
        'date': 'not-a-date',
        'prayer': 'fajr',
      });
    ...
    test('a defective record is silently excluded from every figure, not counted', () {
      final PrayTrackHarness h = PrayTrackHarness();
      addTearDown(h.dispose);
      h.raw(PrayTrackCollections.checkins, 'dddddddd-dddd-4ddd-8ddd-dddddddddddd', <String, Object?>{
        'schema': kPrayTrackSchema,
        'date': kToday.toIso(),
        'prayer': 'made-up',
      });
  ...
  group('toggle — one check-in per (day, prayer)', () {
    ...
    test('toggling it again removes it', () {
      ...
      addTearDown(h.dispose);
      h.toggle(PrayerKey.asr, kToday);
      final PrayTrackResult<PrayTrackWrite> r = h.tryToggle(PrayerKey.asr, kToday);
      expect(r.ok, isTrue);
    ...
    test('undo of a toggle restores the previous state', () {
      final PrayTrackHarness h = PrayTrackHarness();
      addTearDown(h.dispose);
      final PrayTrackResult<PrayTrackWrite> r = h.tryToggle(PrayerKey.dhuhr, kToday);
  ...
  group('doneToday — real, never the reference\'s "time has passed" literal', () {
  ...
  group('current streak — consecutive complete days, never a literal', () {
    ...
    test('stays alive, counting from yesterday, when today is not complete yet', () {
      final PrayTrackHarness h = PrayTrackHarness();
      addTearDown(h.dispose);
      h.completeDay(d(9, 5));
      h.completeDay(d(9, 6));
  ...
  group('best streak — the longest complete run ever, not just the live one', () {
  ...
  group('this month — real, over the reader\'s own check-ins', () {
    test('counts only prayers within today\'s calendar month, through today', () {
      final PrayTrackHarness h = PrayTrackHarness();
      addTearDown(h.dispose);
      h.toggle(PrayerKey.fajr, LumeDate(2026, 8, 31)); // last day of August — excluded
  ...
  group('qada — a real backlog of unmarked past prayers, never the reference\'s bare 7', () {
    test('is 0 with no check-in history at all — never a fabricated starting balance', () {
      final PrayTrackHarness h = PrayTrackHarness();
      addTearDown(h.dispose);
    ...
    test('counts every prayer left unmarked on a day before today, since the first check-in', () {
      final PrayTrackHarness h = PrayTrackHarness();
      addTearDown(h.dispose);
    ...
    test('today\'s unmarked prayers are never counted as qada — the day is not over', () {
      final PrayTrackHarness h = PrayTrackHarness();
      addTearDown(h.dispose);
      h.toggle(PrayerKey.fajr, kToday);
      expect(h.stats().qada, 0);
  ...
  group('heat — the reader\'s own last 35 days, never a random fixture', () {
    test('is 35 days, oldest first, ending today', () {
      final PrayTrackStats s = PrayTrackStats.compute(byDate: <LumeDate, Set<PrayerKey>>{}, today: kToday);
  ...
  group('by prayer — real counts over the heat window, never the reference\'s bare array', () {
    test('counts exactly how many of the last 35 days each prayer was marked', () {
      final PrayTrackHarness h = PrayTrackHarness();
      addTearDown(h.dispose);
      h.toggle(PrayerKey.fajr, kToday);
      h.toggle(PrayerKey.fajr, d(9, 6));
  ...
  group('toggling a defective collection still surfaces failures, not a crash', () {
    test('toggle fails cleanly if the store cannot commit', () {
      final PrayTrackHarness h = PrayTrackHarness(readDelay: const Duration(seconds: 1));
      addTearDown(h.dispose);
      ...
      final PrayTrackResult<PrayTrackWrite> r = h.repo.toggle(PrayerKey.fajr, kToday);
      expect(r.ok, isFalse);
