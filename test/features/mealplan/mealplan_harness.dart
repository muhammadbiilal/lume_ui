/// Meal Plan over an in-memory store, a controllable clock and seeded
...
class MealPlanHarness {
  MealPlanHarness({int seed = 1, Duration? readDelay}) {
  ...
  MealPlanWeek week([LumeDate? today]) => repo.view().week(today ?? kToday);
  ...
  MealPlanEntry set(LumeDate date, MealSlot slot, String text) {
    ...
    expect(r.failure, isNull, reason: 'set $date $slot');
