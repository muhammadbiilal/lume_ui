...
void main() {
  group('model and codec', () {
    ...
    test('a record of the wrong schema is a defect, not a crash', () {
      final CycleHarness h = CycleHarness();
      addTearDown(h.dispose);
      h.raw('cycle.period', 'not-a-uuid', <String, Object?>{'schema': 'lume.other/1'});
  ...
  group('validation', () {
    test('an end before the start is refused', () {
      final CycleHarness h = CycleHarness();
      addTearDown(h.dispose);
      final CycleResult<CycleWrite> r = h.repo.logPeriod(kToday, end: kToday.addDays(-1));
      expect(r.ok, isFalse);
  ...
  group('update', () {
    ...
    test('a stale version is a conflict, not a silent overwrite', () {
      ...
      h.repo.updatePeriod(p.id, start: p.startDate, endDate: kToday.addDays(3), expectVersion: p.version);
  ...
  group('delete and undo', () {
    test('deleting removes the period', () {
      ...
      addTearDown(h.dispose);
      final CyclePeriod p = h.log(kToday);
      final CycleResult<CycleWrite> r = h.repo.deletePeriod(p.id, expectVersion: p.version);
      expect(r.ok, isTrue);
      expect(h.repo.view().periods, isEmpty);
    });
    ...
    test('undo restores a deleted period', () {
      ...
      addTearDown(h.dispose);
      final CyclePeriod p = h.log(kToday);
      final CycleResult<CycleWrite> deleted = h.repo.deletePeriod(p.id, expectVersion: p.version);
  ...
  group('CycleInsights.compute — the reference has no equivalent to match', () {
    test('no periods logged: nothing to show, nothing invented', () {
      final CycleInsights i = CycleInsights.compute(periods: const <CyclePeriod>[], today: kToday);
    ...
    test('one period logged: a current day, but no average and no prediction', () {
      final CycleHarness h = CycleHarness();
      addTearDown(h.dispose);
    ...
    test('a start dated after "today" leaves currentDay unknown, not negative', () {
      final CyclePeriod future = CyclePeriod(
        id: LumeRecordId.generate(),
        startDate: kToday.addDays(3),
        createdAt: DateTime.utc(2026),
      );
      final CycleInsights i = CycleInsights.compute(periods: <CyclePeriod>[future], today: kToday);
      expect(i.currentDay, isNull);
    });

    test('two periods 28 days apart: the average, the estimate and the history are all real', () {
      final CycleHarness h = CycleHarness();
      addTearDown(h.dispose);
    ...
    test('three cycles of uneven length: the mean of the real intervals, not a round number', () {
      final CycleHarness h = CycleHarness();
      addTearDown(h.dispose);
    ...
    test('overdue: a predicted date already passed is a negative countdown, not silence', () {
      final CycleHarness h = CycleHarness();
      addTearDown(h.dispose);
      final LumeDate p1 = LumeDate(2026, 6, 1);
      final LumeDate p2 = LumeDate(2026, 6, 29); // +28
    ...
    test('duplicate or out-of-order spans never poison the average (defensive, not reachable via the repository)', () {
  ...
  group('CycleInsights.phaseFor — the reference\'s own day <= 5/13/16 thresholds, scaled', () {
    test('at the reference\'s own length (28), the thresholds are exactly its own', () {
    ...
    test('a day past the average length still resolves — overdue, not out of range', () {
