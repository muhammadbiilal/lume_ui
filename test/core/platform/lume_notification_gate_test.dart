...
void main() {
  group('Android facts, classified', () {
    LumeNotificationAccess of(LumeNotificationFacts f) => LumeNotificationAccess.classify(f);

    test('granted and interrupted', () {
      expect(of(const LumeNotificationFacts(granted: true)), LumeNotificationAccess.granted);
      expect(
    ...
    test('a check: first request, denied, blocked, undetermined', () {
      expect(of(const LumeNotificationFacts()), LumeNotificationAccess.firstRequest);
      expect(
    ...
    test('a request refused: will ask again, stopped asking, or unknown', () {
      expect(
        of(const LumeNotificationFacts(requested: true, asked: true, rationale: true)),
        LumeNotificationAccess.denied,
      );
      expect(
        of(const LumeNotificationFacts(requested: true, asked: true, rationaleBefore: true)),
        LumeNotificationAccess.blocked,
      );
      expect(
    ...
    test('below API 33: granted is reported true unconditionally by the native side, not guessed here', () {
      // classify() only ever sees what the platform reports; a pre-33
      // device's native side is expected to send granted: true itself.
      expect(of(const LumeNotificationFacts(granted: true, sdk: 30)), LumeNotificationAccess.granted);
    });
  });

  group('iOS facts, classified', () {
    LumeNotificationAccess of(LumeIosNotificationFacts f) => LumeNotificationAccess.classifyIos(f);

    test('granted, not determined, denied', () {
      expect(of(const LumeIosNotificationFacts(granted: true)), LumeNotificationAccess.granted);
      expect(
        of(const LumeIosNotificationFacts(notDetermined: true)),
        LumeNotificationAccess.firstRequest,
      );
      expect(of(const LumeIosNotificationFacts(denied: true)), LumeNotificationAccess.blocked);
    });
  });
  ...
  group('Settings is offered only where it can change the state', () {
    test('blocked and undetermined, and nothing else', () {
      expect(
        ...
        <LumeNotificationAccess>[LumeNotificationAccess.blocked, LumeNotificationAccess.undetermined],
      );
    });
  });
  ...
  group('the Android gate, over its channel', () {
    ...
    test('a request is classified from what the channel reports, exact-alarm carried through', () async {
      ...
      final LumeNotificationState s = await const LumeAndroidNotificationGate().request();
    ...
    test('openExactAlarmSettings calls the channel and never throws outward', () async {
      final List<String> calls = <String>[];
      answer((MethodCall c) {
        calls.add(c.method);
        return null;
  ...
  group('the iOS gate, over its own channel', () {
    const MethodChannel channel = MethodChannel('lume/notification_permission_ios');
    final TestWidgetsFlutterBinding binding = TestWidgetsFlutterBinding.ensureInitialized();
    tearDown(
      () => binding.defaultBinaryMessenger.setMockMethodCallHandler(channel, null),
    );

    test('classified from the channel, and exact-alarm is always available', () async {
      binding.defaultBinaryMessenger.setMockMethodCallHandler(
        channel,
        (MethodCall call) async => <String, Object?>{'denied': true},
      );
      final LumeNotificationState s = await const LumeIosNotificationGate().check();
      expect(s.access, LumeNotificationAccess.blocked);
  ...
  group('unavailable — a platform this feature has no gate for', () {
    test('always reports unavailable, and openExactAlarmSettings is a no-op', () async {
      const LumeUnavailableNotificationGate gate = LumeUnavailableNotificationGate();
      expect((await gate.check()).access, LumeNotificationAccess.unavailable);
      expect((await gate.request()).access, LumeNotificationAccess.unavailable);
