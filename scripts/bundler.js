/* ============================================================
   Lume — the module bundler

   The application is ES modules, and two things cannot load
   them: jsdom, which has no module loader at all, and a browser
   opening a page from file://, which refuses a module script on
   that origin.

   So this resolves the graph itself. It reads each module from
   disk, orders it after everything it imports, rewrites the
   import/export syntax into a small registry, and produces one
   ordinary script.
   ...
   It is honest about what that costs:
     ...
     · The graph MUST be acyclic. Imports are destructured at
       ...
       undefined binding instead of a live one. This throws on a
       cycle rather than producing a bundle that half-works, and
       tests/architecture.js asserts the same rule against the
       real source as a first-class check.
     · Only the syntax Lume actually writes is supported. An
       unrecognised import/export form throws by name rather
       than being silently dropped, and nothing may reach the
       output still spelt as a module.

   Served over HTTP the browser loads the real modules and none
   of this is involved. tests/architecture.js walks the graph
   over a real HTTP server too, so a specifier that resolves
   here but 404s there cannot pass.
   ============================================================ */
const fs = require('fs');
...
module.exports = { bundle, order, transform, codeMatches, IMPORT_RE, resolveSpec };
