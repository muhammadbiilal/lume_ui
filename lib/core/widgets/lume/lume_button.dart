...
/// `.ghostbtn` — a quiet 30-point control that sits inside a card's footer.
///
/// Not [LumeIconButton], which is `.iconbtn`: 38 points, a card fill and a
/// border. This has neither — it is a glyph with a pressed tint, and it is
/// what the quote card's bookmark and share are.
///
/// 30 is under §9's 44-point floor, so the *target* is padded out to 44 while
/// the drawn control stays 30. The card has the room; the reference does not
/// take it, and a bookmark that misses is a bookmark nobody uses.
class LumeGhostButton extends StatelessWidget {
  const LumeGhostButton({
    super.key,
    required this.icon,
  ...
  /// Shown beside the glyph where the control has a word as well.
  final String? label;
  ...
  /// `.ghostbtn { height: 30px }`.
  static const double height = 30;

  @override
  Widget build(BuildContext context) {
    final LumeColors lume = context.lume;
    final Color tint = selected ? lume.accent : lume.text2;

    return LumePressable(
      onTap: onPressed,
      semanticLabel: semanticLabel,
      borderRadius: LumeRadius.brXs,
      // 30, not the 44 §9 asks for — the same call `LumeHeaderButton` makes
      // and for the same reason. `.ghostbtn` is 30 tall and about 35 wide, and
      // it sits in a card foot that is 30 tall; growing it to 44 makes every
      // card carrying one 14 points taller and pushes everything below it down
      // the page. The box is what is drawn *and* what is laid out, so the
      // target cannot be separated from it the way D6 separates the stepper's.
      // Recorded in `docs/LUME_DESTINATIONS.md`.
      minSize: 0,
      child: SizedBox(
        height: height,
        child: Padding(
          ...
          child: Row(
            ...
            children: <Widget>[
              LumeIcon(icon, size: 15, color: tint),
