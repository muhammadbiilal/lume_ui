...
///   camera permission is asked by the camera plugin when the page opens —
///   that is, only after Scan is pressed.
...
/// What a camera error means for the reader. The camera plugin's own codes:
/// iOS says when it will not prompt again (`…WithoutPrompt`) or is restricted;
/// Android says only `CameraAccessDenied`, and [LumeCapturePage] tells the
/// two apart by whether a prompt was actually shown.
@visibleForTesting
LumeScanOutcome lumeCameraTrouble(Object error) {
  ...
  return switch (code) {
    'CameraAccessDenied' => LumeScanOutcome.denied,
    'CameraAccessDeniedWithoutPrompt' ||
    'CameraAccessRestricted' => LumeScanOutcome.blocked,
  ...
  return zxing.ReaderWidget(
    ...
    onControllerCreated: (_, Exception? error) {
      if (error == null) {
        ...
        events.onTrouble(lumeCameraTrouble(error));
      }
    },
    tryInverted: true,
